...
public class AreaStrike : MonoBehaviour
{
    ...
    {
        // 효과범위가 0보다 크지 않다면 범위공격은 작동하지 않습니다.
        if (radius > 0)
        {
            ...
            if (force >= 0 != true)
            {
                Debug.LogError("Wrong value on force" + force);
            }
            ...
            foreach (Collider2D col in cols)
            {
                if (col.gameObject.GetComponent<Unit>())
                {
                    ...
                    if (minimumForceRate >= 0 && minimumForceRate <= 100)
                    {
                        ...
                        actualStrike.force = actualStrike.force * (double)forceMod;
                        if (actualStrike.force >= 0 != true)
                            Debug.LogError("CAUGHT!" + actualStrike.force + forceMod + distance + radius);
                    }
